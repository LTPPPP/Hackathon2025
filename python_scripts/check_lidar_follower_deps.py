...
def check_project_files():
    ...
    required_files = [
        "python_scripts/opposite_detector.py",
        "python_scripts/map_navigator.py", 
        "python_scripts/ros_lidar_follower.py",
        "jetbot/lidar.py"
    ]
...
def check_optional_files():
    ...
    optional_files = {
        ...
        "config/map.json": "Map configuration file",
...
def create_minimal_config_files():
    ...
    if not os.path.exists("config/map.json"):
        minimal_map = {
            "nodes": {
                "start": {"x": 0, "y": 0},
                "goal": {"x": 5, "y": 5}
        ...
        print("✓ Created minimal config/map.json")
    else:
        print("✓ config/map.json already exists")
...
def test_basic_functionality():
    ...
    try:
        ...
        if os.path.exists("config/map.json"):
            navigator = MapNavigator("config/map.json")
            print("✓ Map Navigator test passed")
        else:
            print("ℹ Map Navigator test skipped (no config/map.json)")
