...
class APIRobotController:
    ...
    def setup_api_config(self):
        ...
        self.submit_symbol_endpoint = self.base_url + "api/sign-submissions/submit"
        self.token = "c0435b497712ae6704789b28b710dc88"
        
        # Cấu hình cho việc gửi symbol data
        self.last_submit_time = time.time()
        self.submit_interval = 5.0  # Gửi mỗi 5 giây
        self.current_node_id = "node_001"  # ID node hiện tại, sẽ được cập nhật theo vị trí
    ...
    def submit_symbol_data(self, symbol_text, node_id=None):
        """Gửi dữ liệu symbol/barcode lên server"""
        try:
            if node_id is None:
            ...
            rospy.loginfo(f"Đang gửi symbol data: {symbol_text} tại node {node_id}...")
            
            response = requests.post(
                self.submit_symbol_endpoint,
                json=data,
            ...
            if response.status_code == 200:
                rospy.loginfo(f"Gửi symbol data thành công! Response: {response.json()}")
                return True
            else:
                rospy.logerr(f"Lỗi khi gửi symbol data: {response.status_code} - {response.text}")
                return False
                
        except Exception as e:
            rospy.logerr(f"Exception khi gửi symbol data: {e}")
            return False

    def check_and_submit_symbol(self):
        """Kiểm tra và gửi symbol data nếu đã đến lúc (mỗi 5 giây)"""
        current_time = time.time()
        if current_time - self.last_submit_time >= self.submit_interval:
            # Ở đây bạn có thể thêm logic để nhận diện symbol/barcode từ camera
            # Hiện tại tôi sẽ gửi dữ liệu mẫu để test
            sample_symbol = f"SYMBOL_{int(current_time)}"  # Dữ liệu symbol mẫu
            
            if self.submit_symbol_data(sample_symbol):
    ...
    def execute_square_navigation(self):
        ...
        for i, command in enumerate(self.navigation_commands):
            ...
            while duration_remaining > 0 and not self.should_stop:
                ...
                # Kiểm tra và gửi symbol data mỗi 5 giây
                self.check_and_submit_symbol()
    ...
    def execute_normal_navigation(self):
        ...
        for i, command in enumerate(self.navigation_commands):
            ...
            # Chạy với kiểm tra dừng mỗi 0.5 giây và gửi symbol data
            duration_remaining = self.STRAIGHT_DURATION
            while duration_remaining > 0 and not self.should_stop:
                sleep_time = min(0.5, duration_remaining)
                time.sleep(sleep_time)
